 ...
 */
typedef bool (*esp_lcd_epaper_panel_cb_t)(const esp_lcd_panel_handle_t handle, const void *edata,
                                          void *user_data);
...
typedef struct {
    esp_lcd_epaper_panel_cb_t
        on_epaper_refresh_done; /*!< Callback invoked when e-paper refresh finishes */
 ...
 *        Need be set using `epaper_panel_set_bitmap_color()` before calling
 ...
 *
 * @note The SSD1681 has two separate vrams. One for black and the other for red(or maybe some other
 * color). Call this function to set which vram the next bitmap would write into. If you set
 * `epaper_panel_invert_color()` to false, then 0 means WHITE.
 *
 * @param[in] panel LCD panel handle
 ...
 *
 * @note The callback function `on_epaper_refresh_done` will only be called right after a
 * `draw_bitmap()` caused refresh finishes.
 * @note You could set the `epaper_panel_callbacks_t->esp_lcd_epaper_panel_cb_t` to NULL to
 * unregister the callback.
 * @attention
 *       The callback function `on_epaper_refresh_done` will be called in isr context, so please
 ...
 * @note You do not have to call this function because e-paper panels usually have waveform lut
 * built-in.
 * @note You could call `disp_on_off(panel_handle, true)` to reset the waveform LUT to the built-in
 * one
 *
 * @param[in] panel LCD panel handle
...
/**
 * @brief Set the refresh mode (partial/full)
 *
 * @note Set to true for partial (fast) refresh using internal LUT.
 * @note Set to false for full refresh.
 * @note This function only sets the internal flag, actual refresh happens when calling
 * epaper_panel_refresh_screen().
 *
 * @param[in] panel LCD panel handle
 * @param[in] partial_refresh true for partial refresh, false for full refresh
 * @return  ESP_OK                on success
 *          ESP_ERR_INVALID_ARG   if parameter is invalid
 */
