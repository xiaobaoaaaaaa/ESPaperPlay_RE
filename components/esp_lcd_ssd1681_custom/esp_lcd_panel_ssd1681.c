...
typedef struct {
    ...
    bool _invert_color;
    bool _partial_refresh; // Flag for partial refresh mode
} epaper_panel_t;

// --- Utility functions
static inline uint8_t byte_reverse(uint8_t data);
static esp_err_t process_bitmap(esp_lcd_panel_t *panel, int len_x, int len_y, int buffer_size,
...
static esp_err_t epaper_set_area(esp_lcd_panel_io_handle_t io, uint32_t start_x, uint32_t start_y,
                                 uint32_t end_x, uint32_t end_y);
static esp_err_t panel_epaper_set_vram(esp_lcd_panel_io_handle_t io, uint8_t *bw_bitmap,
                                       uint8_t *red_bitmap, size_t size);
// --- SSD1681 specific functions, exported to user in public header file
// extern esp_err_t esp_lcd_new_panel_ssd1681(const esp_lcd_panel_io_handle_t io, const
// esp_lcd_panel_dev_config_t *panel_dev_config,
//                                            esp_lcd_panel_handle_t *ret_panel);
// extern esp_err_t epaper_panel_register_event_callbacks(esp_lcd_panel_t *panel,
// epaper_panel_callbacks_t* cbs, void* user_ctx); extern esp_err_t
// epaper_panel_refresh_screen(esp_lcd_panel_t *panel); extern esp_err_t
// epaper_panel_set_bitmap_color(esp_lcd_panel_t* panel, esp_lcd_ssd1681_bitmap_color_t color);
...
static esp_err_t epaper_panel_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start,
...
static void epaper_driver_gpio_isr_handler(void *arg) {
    ...
    if (epaper_panel->epaper_refresh_done_isr_callback.callback_ptr) {
        (epaper_panel->epaper_refresh_done_isr_callback.callback_ptr)(
            &(epaper_panel->base), NULL, epaper_panel->epaper_refresh_done_isr_callback.args);
    }
}
...
static esp_err_t epaper_set_lut(esp_lcd_panel_io_handle_t io, const uint8_t *lut) {
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_DISP_UPDATE_CTRL, (uint8_t[]){lut[153]}, 1),
        TAG, "SSD1681_CMD_SET_END_OPTION err");

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_GATE_DRIVING_VOLTAGE,
                                                  (uint8_t[]){lut[154]}, 1),
                        TAG, "SSD1681_CMD_SET_END_OPTION err");

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_SRC_DRIVING_VOLTAGE,
                                                  (uint8_t[]){lut[155], lut[156], lut[157]}, 3),
                        TAG, "SSD1681_CMD_SET_SRC_DRIVING_VOLTAGE err");

    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_VCOM_REG, (uint8_t[]){lut[158]}, 1), TAG,
        "SSD1681_CMD_SET_VCOM_REG err");

    return ESP_OK;
}

static esp_err_t epaper_set_cursor(esp_lcd_panel_io_handle_t io, uint32_t cur_x, uint32_t cur_y) {
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_INIT_X_ADDR_COUNTER,
                                                  (uint8_t[]){(uint8_t)((cur_x >> 3) & 0xff)}, 1),
                        TAG, "SSD1681_CMD_SET_INIT_X_ADDR_COUNTER err");

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_INIT_Y_ADDR_COUNTER,
                                                  (uint8_t[]){
                                                      (uint8_t)(cur_y & 0xff),       // cur_y[7:0]
                                                      (uint8_t)((cur_y >> 8) & 0xff) // cur_y[8]
                                                  },
                                                  2),
                        TAG, "SSD1681_CMD_SET_INIT_Y_ADDR_COUNTER err");
...
static esp_err_t epaper_set_area(esp_lcd_panel_io_handle_t io, uint32_t start_x, uint32_t start_y,
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_RAMX_START_END_POS,
                                                  (uint8_t[]){
                                                      (start_x >> 3) & 0xff, // start_x
                                                      (end_x >> 3) & 0xff    // end_x
                                                  },
                                                  2),
                        TAG, "SSD1681_CMD_SET_RAMX_START_END_POS err");

    // --- Set RAMY Start/End Position
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_SET_RAMY_START_END_POS,
                                                  (uint8_t[]){
...
esp_err_t panel_epaper_set_vram(esp_lcd_panel_io_handle_t io, uint8_t *bw_bitmap,
                                uint8_t *red_bitmap, size_t size) {
    // Note: the screen region to be used to draw bitmap had been defined
    // The region of BLACK VRAM and RED VRAM are set by the same series of command, the two bitmaps
    // will be drawn at the same region, so the two bitmaps can share a same size.
    // --- Validate and Transfer data
    if (bw_bitmap && (size > 0)) {
        // tx WHITE/BLACK bitmap
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_color(io, SSD1681_CMD_WRITE_BLACK_VRAM, bw_bitmap, size), TAG,
            "data bw_bitmap err");
    }
    if (red_bitmap && (size > 0)) {
        // tx RED bitmap
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_color(io, SSD1681_CMD_WRITE_RED_VRAM, red_bitmap, size), TAG,
            "data red_bitmap err");
    }
    return ESP_OK;
}
...
esp_err_t epaper_panel_refresh_screen(esp_lcd_panel_t *panel) {
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_DISP_UPDATE_CTRL,
                                                  (uint8_t[]){
                                                      duc_flag // Color invert flag
                                                  },
                                                  1),
                        TAG, "SSD1681_CMD_DISP_UPDATE_CTRL err");

    // --- Set Border Waveform based on refresh mode
    if (epaper_panel->_partial_refresh) {
        // Partial refresh: use 0x80 for border waveform
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_BORDER_WAVEFORM,
                                      (uint8_t[]){SSD1681_PARAM_BORDER_WAVEFORM_PARTIAL}, 1),
            TAG, "SSD1681_CMD_SET_BORDER_WAVEFORM err");
    } else {
        // Full refresh: use 0x01 for border waveform
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_BORDER_WAVEFORM,
                                      (uint8_t[]){SSD1681_PARAM_BORDER_WAVEFORM}, 1),
            TAG, "SSD1681_CMD_SET_BORDER_WAVEFORM err");
    }

    // --- Enable refresh done handler isr
    gpio_intr_enable(epaper_panel->busy_gpio_num);
    // --- Send refresh command: select update mode based on partial/full refresh
    if (epaper_panel->_partial_refresh) {
        // Partial refresh: use 0xFF for fast update with internal LUT
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_DISP_UPDATE_CTRL,
                                      (uint8_t[]){SSD1681_PARAM_DISP_PARTIAL_REFRESH}, 1),
            TAG, "SSD1681_CMD_SET_DISP_UPDATE_CTRL err");
    } else {
        // Full refresh: use 0xCF for standard update
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_DISP_UPDATE_CTRL,
                                      (uint8_t[]){SSD1681_PARAM_DISP_WITH_MODE_2}, 1),
            TAG, "SSD1681_CMD_SET_DISP_UPDATE_CTRL err");
    }

    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_ACTIVE_DISP_UPDATE_SEQ, NULL, 0),
        TAG, "SSD1681_CMD_ACTIVE_DISP_UPDATE_SEQ err");

    return ESP_OK;
}

esp_err_t epaper_panel_set_refresh_mode(esp_lcd_panel_t *panel, bool partial_refresh) {
    ESP_RETURN_ON_FALSE(panel, ESP_ERR_INVALID_ARG, TAG, "panel handler is NULL");
    epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
    epaper_panel->_partial_refresh = partial_refresh;
    return ESP_OK;
}
...
esp_err_t esp_lcd_new_panel_ssd1681(const esp_lcd_panel_io_handle_t io,
    ...
    epaper_panel->_partial_refresh = false; // Default to full refresh
    ...
    *ret_panel = &(epaper_panel->base);
    ...
    if (!(epaper_panel->_non_copy_mode)) {
        epaper_panel->_framebuffer = heap_caps_malloc(
            SSD1681_EPD_1IN54_V2_WIDTH * SSD1681_EPD_1IN54_V2_HEIGHT / 8, MALLOC_CAP_DMA);
...
static esp_err_t epaper_panel_reset(esp_lcd_panel_t *panel) {
    ...
    if (epaper_panel->reset_gpio_num >= 0) {
        ...
        ESP_RETURN_ON_ERROR(
            gpio_set_level(epaper_panel->reset_gpio_num, !epaper_panel->reset_level), TAG,
...
esp_err_t epaper_panel_set_bitmap_color(esp_lcd_panel_t *panel,
                                        esp_lcd_ssd1681_bitmap_color_t color) {
    ESP_RETURN_ON_FALSE(panel, ESP_ERR_INVALID_ARG, TAG, "panel handler is NULL");
    epaper_panel_t *epaper_panel = __containerof(panel, epaper_panel_t, base);
...
static esp_err_t epaper_panel_init(esp_lcd_panel_t *panel) {
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_OUTPUT_CTRL,
                                                  SSD1681_PARAM_OUTPUT_CTRL, 3),
                        TAG, "SSD1681_CMD_OUTPUT_CTRL err");

    // --- Border Waveform Control
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_BORDER_WAVEFORM,
                                                  (uint8_t[]){SSD1681_PARAM_BORDER_WAVEFORM}, 1),
                        TAG, "SSD1681_CMD_SET_BORDER_WAVEFORM err");

    // --- Temperature Sensor Control
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_TEMP_SENSOR,
                                                  (uint8_t[]){SSD1681_PARAM_TEMP_SENSOR}, 1),
                        TAG, "SSD1681_CMD_SET_TEMP_SENSOR err");
    // --- Load built-in waveform LUT
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io,
                                                  SSD1681_CMD_SET_DISP_UPDATE_CTRL,
                                                  (uint8_t[]){SSD1681_PARAM_DISP_UPDATE_MODE_1}, 1),
                        TAG, "SSD1681_CMD_SET_DISP_UPDATE_CTRL err");
    // --- Display end option
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SET_END_OPTION,
                                                  (uint8_t[]){SSD1681_PARAM_END_OPTION_KEEP}, 1),
                        TAG, "SSD1681_CMD_SET_END_OPTION err");
    // --- Active Display Update Sequence
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, SSD1681_CMD_ACTIVE_DISP_UPDATE_SEQ, NULL, 0),
                        TAG, "param SSD1681_CMD_SET_DISP_UPDATE_CTRL err");
    panel_epaper_wait_busy(panel);
...
static esp_err_t epaper_panel_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start,
    ...
    if (epaper_panel->_non_copy_mode) {
        ...
        if (!esp_ptr_dma_capable(epaper_panel->_framebuffer)) {
            ESP_LOGW(
                TAG,
                "Bitmap not DMA capable, use DMA capable memory to avoid additional data copy.");
        }
    } else {
    ...
    if ((!(epaper_panel->_mirror_x)) && (!(epaper_panel->_mirror_y))) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_DATA_ENTRY_MODE,
                                                      (uint8_t[]){SSD1681_PARAM_DATA_ENTRY_MODE_3},
                                                      1),
                            TAG, "SSD1681_CMD_DATA_ENTRY_MODE err");
    }
    if ((!(epaper_panel->_mirror_x)) && (epaper_panel->_mirror_y)) {
        // --- Cursor Settings
        ESP_RETURN_ON_ERROR(epaper_set_area(epaper_panel->io, x_start, y_start + len_y - 1, x_end,
                                            y_end + len_y - 1),
        ...
        // --- Data Entry Sequence Setting
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_DATA_ENTRY_MODE,
                                                      (uint8_t[]){SSD1681_PARAM_DATA_ENTRY_MODE_1},
                                                      1),
                            TAG, "SSD1681_CMD_DATA_ENTRY_MODE err");
    }
    if (((epaper_panel->_mirror_x)) && (!(epaper_panel->_mirror_y))) {
...
static esp_err_t epaper_panel_disp_on_off(esp_lcd_panel_t *panel, bool on_off) {
    ...
    if (on_off) {
        ...
        ESP_RETURN_ON_ERROR(
            ...
            TAG, "SSD1681_CMD_SET_DISP_UPDATE_CTRL err");
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(io, SSD1681_CMD_ACTIVE_DISP_UPDATE_SEQ, NULL, 0), TAG,
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(epaper_panel->io, SSD1681_CMD_SLEEP_CTRL,
                                                      (uint8_t[]){SSD1681_PARAM_SLEEP_MODE_1}, 1),
                            TAG, "SSD1681_CMD_SLEEP_CTRL err");
        ...
        if (!(epaper_panel->_non_copy_mode)) {
            if (epaper_panel->_swap_xy) {
                ...
                for (int i = 0; i < buffer_size * 8; i++) {
                    ...
                    (epaper_panel
                         ->_framebuffer)[((i * len_y / 8) % buffer_size) + (i / 8 / len_x)] |=
    ...
    if ((!(epaper_panel->_mirror_x)) && (epaper_panel->_mirror_y)) {
        if (epaper_panel->_swap_xy) {
            ...
            for (int i = 0; i < buffer_size * 8; i++) {
                uint8_t bitmap_byte = ((uint8_t *)(color_data))[i / 8];
                uint8_t bitmap_pixel = (bitmap_byte & (0x01 << (7 - (i % 8)))) ? 0x01 : 0x00;
                (epaper_panel->_framebuffer)[buffer_size -
                                             (((i * len_y / 8) % buffer_size) + (i / 8 / len_x)) -
...
static inline uint8_t byte_reverse(uint8_t data) {
    static uint8_t _4bit_reverse_lut[] = {0x00, 0x08, 0x04, 0x0C, 0x02, 0x0A, 0x06, 0x0E,
