...
// 适用于 FTxxxx 的 I2C读
static void i2c_ctp_FTxxxx_read(i2c_port_t i2c_num, uint8_t reg_addr, uint8_t *data_rd,
                                size_t size) {
    i2c_master_read_slave_reg(i2c_num, CTP_FTxxxx_SLAVE_ADDRESS, reg_addr, data_rd, size,
                              CTP_FTxxxx_TICKS_TO_WAIT);
}

// 适用于 FTxxxx 的 I2C写
static void i2c_ctp_FTxxxx_write(i2c_port_t i2c_num, uint8_t reg_addr, uint8_t *data_wr,
                                 size_t size) {
    i2c_master_write_slave_reg(i2c_num, CTP_FTxxxx_SLAVE_ADDRESS, reg_addr, data_wr, size,
                               CTP_FTxxxx_TICKS_TO_WAIT);
}
...
void i2c_ctp_FTxxxx_read_all(i2c_port_t i2c_num, ctp_tp_t *ctp) {
    ...
    // 从31字节中，解析触摸发生的总个数
    if (tmp[0] ==
        0xFF) { // 读取触摸发生的总个数。处理异常，刚上电后无触摸会读取出 0xFF。有触摸后值正常。
        tmp[0] = 0;
    }
    ...
    for (uint8_t i = 0; i < tmp[0]; i++) {
        ...
        ctp->tp[i].x =
            (((uint16_t)tmp[1 + i * 6] & 0x0F) << 8) | tmp[2 + i * 6]; // 触摸产生的点的X坐标
        ctp->tp[i].y =
            (((uint16_t)tmp[3 + i * 6] & 0x0F) << 8) | tmp[4 + i * 6]; // 触摸产生的点的Y坐标
