 ...
 *     - none
 */
esp_err_t i2c_master_init(i2c_port_t i2c_num, uint32_t clk_speed, gpio_num_t scl_io_num,
    ...
    return i2c_driver_install(i2c_num, conf_master.mode, I2C_MASTER_RX_BUF_DISABLE,
 ...
 *     - none
 */
esp_err_t i2c_slave_init(i2c_port_t i2c_num, uint8_t slave_addr, gpio_num_t scl_io_num,
                         gpio_num_t sda_io_num) {
    ...
    return i2c_driver_install(i2c_num, conf_slave.mode, I2C_SLAVE_RX_BUF_LEN, I2C_SLAVE_TX_BUF_LEN,
                              0);
}
 ...
 * @brief  I2Cx-读从设备的值
 *      - 不带有读器件寄存器的方式，适用于
 * BH1750、ADS1115/1118等少数I2C设备，这类设备通常内部寄存器很少
 ...
 *
 */
esp_err_t i2c_master_read_slave(i2c_port_t i2c_num, uint8_t slave_addr, uint8_t *data_rd,
 ...
 * | start | slave_addr + rd_bit + ack | reg_addr + ack | start | slave_addr + wr_bit + ack | read
 * n-1 bytes + ack | read 1 byte + nack | stop |
 * --------|---------------------------|------------------------|---------------------------|----------------------|--------------------|------|
 *
 * @param  i2c_num I2C端口号。I2C_NUM_0 / I2C_NUM_1
 ...
 *     - esp_err_t
 */
esp_err_t i2c_master_read_slave_reg(i2c_port_t i2c_num, uint8_t slave_addr, uint8_t reg_addr,
                                    uint8_t *data_rd, size_t size, TickType_t ticks_to_wait) {
    if (size == 0) {
        return ESP_OK;
    }
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
 ...
 *      - 例：i2c_master_read_slave_reg_16bit(I2C_NUM_0, 0x44, 0xE000, &test, 6, 100 /
 * portTICK_RATE_MS);
 *
 * ____________________________________________________________________________________________________________________________________________________
 * | start | slave_addr + rd_bit + ack | reg_addr(2byte) + ack | start | slave_addr + wr_bit + ack |
 * read n-1 bytes + ack | read 1 byte + nack | stop |
 * --------|---------------------------|-------------------------------|---------------------------|----------------------|--------------------|------|
 *
 * @param  i2c_num I2C端口号。I2C_NUM_0 / I2C_NUM_1
 * @param  slave_addr I2C读从机的器件地址
 ...
 *     - esp_err_t
 */
esp_err_t i2c_master_read_slave_reg_16bit(i2c_port_t i2c_num, uint8_t slave_addr, uint16_t reg_addr,
                                          uint8_t *data_rd, size_t size, TickType_t ticks_to_wait) {
    if (size == 0) {
        return ESP_OK;
    }
 ...
 *
 */
esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t slave_addr, uint8_t *data_wr,
                                 size_t size, TickType_t ticks_to_wait) {
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (slave_addr << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN);
